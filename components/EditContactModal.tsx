...
interface EditContactModalProps {
  ...
  visible: boolean;
  onClose: () => void;
  onSave: (newBucket: Contact['bucket'], customIntervalDays?: number | null) => void;
}
...
const formatCustomSummary = (days?: number | null) => {
  if (!days || days < 1) return 'Set a custom cadence';
...
export default function EditContactModal({ contact, visible, onClose, onSave }: EditContactModalProps) {
  const [selectedBucket, setSelectedBucket] = useState<Contact['bucket']>(contact.bucket);
  const [{ customUnit, customValue }, setCustomState] = useState(() => deriveCustomUnitAndValue(contact.customIntervalDays));
  ...
  const isCustomValid = !isCustom || (derivedCustomDays !== null && derivedCustomDays >= 1 && derivedCustomDays <= 365);
  const hasChanges =
      ...
      <SafeAreaView className="flex-1 bg-cream">
        <View className="flex-1 px-6 pb-4 pt-6">
          ...
          <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
            ...
            <Text className="mb-2 text-base text-slate-500">How often would you like to check in with {contact.name}?</Text>

            <Text className="mb-3 text-base font-semibold text-slate">Contact Reminders</Text>

            <View className="mb-4 flex gap-2">
              {(['daily', 'weekly', 'bi-weekly', 'monthly', 'yearly', 'custom'] as Contact['bucket'][]).map((bucket) => (
                <View key={bucket}>
                  <Pressable
                      ...
                      <View className="flex-1">
                        <Text
                          className={`text-base font-semibold ${
                            selectedBucket === bucket ? 'text-slate' : 'text-slate-700'
                          }`}
                        >
                          {bucketLabels[bucket]}
                        </Text>
                        <Text className="mt-1 text-sm text-slate-500">
                          {bucket === 'custom'
                            ? formatCustomSummary(derivedCustomDays ?? contact.customIntervalDays)
                    ...
                    <View className="rounded-b-2xl border-x-2 border-b-2 border-sage bg-white px-4 pb-4 pt-2">
                      <View className="mt-2 flex-col gap-3">
                        <View>
                          ...
                          <TextInput
                            value={customValue}
                            onChangeText={(text) => setCustomState({ customUnit, customValue: text.replace(/[^0-9]/g, '') })}
                            ...
                            {(['days', 'weeks', 'months'] as CustomUnit[]).map((unit) => (
                              ...
                              >
                                <Text className={`text-sm font-medium ${
                                  customUnit === unit ? 'text-slate-900' : 'text-slate-500'
                                }`}>
                      ...
                      {!isCustomValid && (
                        <Text className="mt-3 text-sm text-rose-500 font-medium">Please enter a valid duration (1-365 days)</Text>
                      )}

                      {isCustomValid && derivedCustomDays && (
                        <Text className="mt-3 text-sm text-slate-600">
                          We’ll remind you <Text className="font-semibold text-sage-700">{formatCustomSummary(derivedCustomDays)}</Text>.
                        </Text>
                      )}
                    </View>
                  )}
